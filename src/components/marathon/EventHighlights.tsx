...
export default function EventHighlights() {
  return (
    <section className="py-20 md:py-24 bg-background dots-pattern">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-14">
          <p className="text-royal font-semibold text-sm tracking-widest uppercase">Every Participant Gets</p>
          <h2 className="mt-3 text-3xl md:text-4xl font-extrabold text-charcoal">Included In Every Bib</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5 md:gap-6">
          {items.map(({ icon: Icon, title, desc, num }) => (
            ...
            >
              ...
              <div className="mx-auto grid place-items-center h-14 w-14 rounded-2xl bg-royal/5 text-royal group-hover:bg-gradient-to-br group-hover:from-royal group-hover:to-navy group-hover:text-white transition-all duration-500 shadow-sm group-hover:shadow-glow">
                <Icon className="h-6 w-6 transition-transform duration-300" strokeWidth={1.75} />
              </div>
              <h3 className="mt-4 font-bold text-charcoal text-sm md:text-base tracking-tight transition-colors duration-300 group-hover:text-royal">{title}</h3>
